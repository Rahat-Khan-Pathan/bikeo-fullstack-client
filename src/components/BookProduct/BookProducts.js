...
const BookProducts = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://immense-sierra-11894.herokuapp.com/products/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setData(data);
        setLoading(false);
          ...
          {loading ? (
            ...
            <>
              <div className="container add-service-container">
                <h1 className="booking-header">Order Product</h1>
                <hr />
                <div className="d-flex justify-content-center">
                  <div className="d-flex justify-content-between align-items-center mb-4 details">
                    <h4 className="price"><span className="sp-color">Product: </span>{data.name}</h4>
                    <h4 className="price"><span className="sp-color">Price: </span>${data.price}</h4>
                  </div>
                </div>
